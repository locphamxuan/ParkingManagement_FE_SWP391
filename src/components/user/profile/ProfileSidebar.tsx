      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4 rounded-3xl bg-slate-950/40 border border-white/5 p-6 shadow-md">
          ...
          <div className="grid gap-3 text-xs text-slate-400">
            ...
            {user.role === 'user' && (
              <div className="rounded-2xl border border-white/5 bg-slate-950/70 p-4">
                <p className="text-[10px] font-black uppercase text-slate-500 font-mono">Phương tiện</p>
                <p className={`mt-1 font-mono font-black text-sm ${vehicles.length > 0 ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {vehicles.length > 0 ? `${vehicles.length}/${MAX_VEHICLES} xe` : 'Chưa có'}
                </p>
              </div>
            )}
          </div>
        ...
        {user.role === 'user' && (
          <div className="rounded-3xl bg-slate-950/40 border border-white/5 p-6 shadow-md space-y-3">
            <h2 className="text-xs font-black uppercase tracking-wider text-slate-400 font-mono">Số chỗ đăng ký xe</h2>
            ...
            <p className="text-[9px] text-slate-500 font-semibold">
              {vehicles.length === 0
                ...
                : vehicles.length < MAX_VEHICLES
                  ? `${MAX_VEHICLES - vehicles.length} chỗ trống còn lại.`
                  : 'Đã đạt giới hạn.'}
            </p>
          </div>
        )}
