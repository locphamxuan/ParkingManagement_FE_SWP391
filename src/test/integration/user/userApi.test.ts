...
describe('User — reservations', () => {
  it('list: trả về danh sách đặt chỗ với pagination', async () => {
    const res = await userApi.reservations.list({ page: 1, limit: 10 });
    expect(res.data).toBeDefined();
    ...
    if (res.data.items.length > 0) {
      ...
      expect(r).toHaveProperty('plateNumber');
      expect(['pending', 'confirmed', 'checked_in', 'expired', 'cancelled', 'completed']).toContain(r.status);
    }
  });

  it('list: filter theo status=cancelled trả về đúng loại', async () => {
    const res = await userApi.reservations.list({ status: 'cancelled', limit: 5 });
    expect(Array.isArray(res.data.items)).toBe(true);
  ...
  it('estimate: trả về phí ước tính + số tiền cọc (15%)', async () => {
    ...
    const building = buildingRes.data.items.find((b) => b.status === 'active');
    if (!building) return;

    const vtRes = await userApi.buildings.vehicleTypes(building._id);
    const vt = vtRes.data.items[0];
    if (!vt) return;

    const startTime = new Date(Date.now() + 60 * 60 * 1000).toISOString(); // 1 tiếng sau
    const endTime = new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(); // 3 tiếng sau

    const res = await userApi.reservations.estimate({
    ...
    expect(res.data).toBeDefined();
    expect(typeof res.data.estimatedFee).toBe('number');
    expect(typeof res.data.depositAmount).toBe('number');
    expect(res.data.estimatedFee).toBeGreaterThanOrEqual(0);
    // depositAmount = depositPercent% của estimatedFee
    if (res.data.estimatedFee > 0) {
      expect(res.data.depositAmount).toBeGreaterThan(0);
    }
  });
});
